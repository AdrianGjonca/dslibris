...
void NewCache::LoadIn(FT_Face face, u32 ucs, FT_GlyphSlot glyph){
	textCache2.insert(std::make_pair(
			std::make_pair(face, (u16) ucs),
			glyph
	));
...
bool NewCache::LoadOut(FT_Face face, u32 ucs, FT_GlyphSlot * glyph){
	std::map<std::pair<FT_Face, u16>, FT_GlyphSlot>::iterator iter = textCache2.find(std::make_pair(face, (u16) ucs));
	
	if (iter != textCache2.end()) {
		//stats_hits++
		//hit = true
		*glyph = iter->second;
		return true;
